...
namespace OxDb.SharedCore.Serialization.Utils
{
    ...
    {
        public static void Init(string dirName, IReflectionService reflectionService, Assembly topLevelAssembly)
        {
            MessagePackInitializer mpi = new MessagePackInitializer();
            mpi.Init(dirName, reflectionService, topLevelAssembly);
            SystemTextJsonInitializer sji = new SystemTextJsonInitializer();
            sji.Init(dirName, reflectionService, topLevelAssembly);
        }
    }
}