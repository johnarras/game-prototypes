...
namespace OxDb.MapServer.Spawns.Services
{

    public interface ISpawnService : IInitializable
    {
        List<RewardList> Roll(IRandom rand, long spawnTableId, long rewardSourceId, RollLootArgs rollLootArgs);
        List<RewardList> Roll(IRandom rand, SpawnTable st, long rewardSourceId, RollLootArgs rollLootArgs);
        List<RewardList> Roll<SI>(IRandom rand, List<SI> items, long rewardSourceId, RollLootArgs rollLootArgs) where SI : ISpawnItem;
    }
    ...
    public class SpawnService : ISpawnService
    {
        ...
        public List<RewardList> Roll(IRandom rand, long spawnTableId, long rewardSourceId, RollLootArgs rollLootArgs)
        {
            return Roll(rand, _gameData.Get<SpawnSettings>(null).Get(spawnTableId), rewardSourceId, rollLootArgs);
        }

        // Different public roll methods.

        public List<RewardList> Roll(IRandom rand, SpawnTable st, long rewardSourceId, RollLootArgs rollLootArgs)
        {
            if (st == null)
            {
            ...
            return Roll(rand, st.Items, rewardSourceId, rollLootArgs);
        }

        public List<RewardList> Roll<SI>(IRandom rand, List<SI> items, long rewardSourceId, RollLootArgs rollLootArgs) where SI : ISpawnItem
        {
            return InnerRoll(rand, items, rewardSourceId, rollLootArgs);
        }

        private List<RewardList> InnerRoll<SI>(IRandom rand, List<SI> items, long rewardSourceId, RollLootArgs rollLootArgs) where SI : ISpawnItem
        {
            List<RewardList> list = new List<RewardList>();

            list.Add(_rewardService.CreateRewardList(rewardSourceId, null, rollLootArgs.EntityId));
            for (int i = 0; i < rollLootArgs.Times; i++)
            {
                rollLootArgs.Depth++;
                list.AddRange(RollOnce(rand, items, rewardSourceId, rollLootArgs));
                rollLootArgs.Depth--;
            }
        ...
        private List<RewardList> RollOnce<SI>(IRandom rand, List<SI> items, long rewardSourceId, RollLootArgs rollLootArgs) where SI : ISpawnItem
        {
            if (items == null)
            {
                return new List<RewardList>();
            ...
            foreach (SI si in items)
            {
                ...
                if (si.GroupId < 1)
                {
                    ...
                    {
                        rollLootArgs.Depth++;
                        retval = retval.Concat(RollOneItem(rand, si, rewardSourceId, rollLootArgs)).ToList();
                        rollLootArgs.Depth--;
                    }
                    continue;
                }
        ...
        private List<RewardList> RollOneItem<SI>(IRandom rand, SI si, long rewardSourceId, RollLootArgs rollLootArgs) where SI : ISpawnItem
        {
            List<RewardList> retval = new List<RewardList>();
            ...
            if (rollHelper != null)
            {
                retval = rollHelper.Roll(rand, rewardSourceId, rollLootArgs, si);
                return retval;
            }


            RewardList rewardList = _rewardService.CreateRewardList(rewardSourceId, new List<Reward>(), rollLootArgs.EntityId);
