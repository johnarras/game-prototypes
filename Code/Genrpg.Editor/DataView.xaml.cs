...
{
    public partial class DataView : UserControlBase, IUICanvas
    {
        ...
        public ButtonBase AddButton = null;
        public ButtonBase DeleteButton = null;
        public ButtonBase CopyButton = null;
        public ButtonBase DetailsButton = null;
        ...
        private IList<CanvasBase> colorPanels = new List<CanvasBase>();
        ...
        private void MultiGrid_SelectionChanged(object sender, object e)
        {
            MarkSelectedRows();
        }
        ...
        public void OnClickAdd(object sender, object e)
        {
            AddItems();
        }

        public void OnClickDelete(object sender, object e)
        {
            DispatcherQueue.TryEnqueue(DeleteItem);
        }


        public void OnClickDetails(object sender, object e)
        {
            SaveChanges();
            ShowDetails();
        }

        public void OnClickBack(object sender, object e)
        {
            SaveChanges();
            if (_window != null)
            {
        ...
        public void OnClickHome(object sender, object e)
        {
            SaveChanges();
            if (_window != null)
            {
        ...
        public void ShowMultiTypeData()
        {
            ...
            UIHelper.SetVisible(_singleGrid, false);
            UIHelper.SetVisible(_multiGrid, true);
            SetMultiGridDataSource(Obj);
            UIHelper.SetVisible(AddButton, true);
            UIHelper.SetVisible(DeleteButton, true);
            UIHelper.SetVisible(CopyButton, true);
            UIHelper.SetVisible(DetailsButton, true);
        ...
        public void ShowSingleTypeData()
        {
            ...
            _singleGrid.ClearChildren();
            ShowMultiItemButtonsOnSingleItemView();
            UIHelper.SetVisible(_singleGrid, true);
            UIHelper.SetVisible(_multiGrid, false);
            UIHelper.SetVisible(AddButton, false);
            UIHelper.SetVisible(DeleteButton, false);
            UIHelper.SetVisible(CopyButton, false);
            UIHelper.SetVisible(DetailsButton, false);
            ...
            for (int i = 0; i < members.Count; i++)
            {
                ...
                if (eb != null)
                {
                    TextBoxBase tbox = eb as TextBoxBase;

                    if (tbox != null && mem.Name.IndexOf("Description") >= 0)
                    {
                        tbox.AcceptsReturn = true;
                        UIHelper.SetSize(eb, sx * 6 / 5, sy * 3 / 2);
                    }
                    else
                    {
                        UIHelper.SetSize(eb, sx * 6 / 5, sy);
                    }
                    ...
                    CheckBoxBase cbox = eb as CheckBoxBase;
                    if (cbox != null)
                    {
                        cbox.Content = "";
        ...
        private object AddSingleControl(MemberInfo mem, IUICanvas coll, double width, double height, double xpos, double ypos)
        {
            ...
            TextBoxBase textBox = UIHelper.CreateTextBoxBase(coll, mem.Name + "Edit", null, width, height, xpos, ypos, null);
            return textBox;
        }

        private void OnEntityTypeIndexChanged(object sender, object e)
        {
            ComboBoxBase cb = sender as ComboBoxBase;
            if (_singleGrid == null)
            {
                return;
            }
            ...
            ComboBoxBase keyCont = null;

            for (int c = 0; c < _singleGrid.GetChildCount(); c++)
            {
                keyCont = _singleGrid.GetChild(c) as ComboBoxBase;
                if (keyCont != null)
                {
                    break;
                }
        ...
        private void OnClickEntityIDDropdown(object sender, object ev)
        {
            ...
            if (shiftIsDown)
            {
                if (currObject != null)
                {
                    UserControlBase dv = dvf.Create(_gs, _window, currObject, datalist, Obj, this);
                }
                return;
            }
            ...
            if (ctrlIsDown)
            {
                UserControlBase uc = dvf.Create(_gs, _window, datalist, Obj, _parent, this);
            ...
            if (altIsDown)
            {
                ...
                if (newObj != null)
                {
                    ...
                    UserControlBase uc = dvf.Create(_gs, _window, newObj, datalist, this, this);
        ...
        private void OnComboBoxBaseChanged(object sender, object e)
        {
            ComboBoxBase cb = sender as ComboBoxBase;
            ...
            if (propName.IndexOf("EntityTypeId") >= 0)
            {
                ...
                if (mem != null)
                {
                    ...
                    object cont = _singleGrid.GetChildNamed(entityControlName);

                    if (cont != null)
                    {

                        _singleGrid.Remove(cont);

                        AddSingleControl(mem, _singleGrid, UIHelper.GetWidth(cont), UIHelper.GetHeight(cont),
                            UIHelper.GetLeft(cont), UIHelper.GetTop(cont));
                    }
                }
            }
        }

        private ComboBoxBase AddDropdownComboBox(Object dropdownList, MemberInfo mem, IUICanvas coll, double xsize, double ysize, double xpos, double ypos)
        {
            ...
            ComboBoxBase comboBox = UIHelper.CreateComboBoxBase(coll, "DropDown", xsize, ysize, xpos, ypos);
        ...
        protected void ShowMultiItemButtonsOnSingleItemView()
        {
            ...
            foreach (MemberInfo mem in members)
            {
                ...
                {
                    int extraWidth = 3;
                    CanvasBase colorPanel = new CanvasBase()
                    {
                    ...
                    _singleGrid.Add(colorPanel, CanvasBase.GetLeft(button) - extraWidth, CanvasBase.GetTop(button) - extraWidth);
        ...
        private void OnClickSingleObjectButton(object childObj)
        {
            ...
            UserControlBase view = ucf.Create(_gs, _window, childObj, Obj, _parent, this);

        }

        private void OnClickSingleTopButton(object sender, object e)
        {
        ...
        public void DeleteItem()
        {
            ...
            foreach (IGameSettings settingsToDelete in deleteItems)
            {
                _repoService.QueueDelete(settingsToDelete);
            }
        }
        ...
        public void ShowDetails()
        {
            ...
            UserControlFactory ucf = new UserControlFactory();
            UserControlBase uc = ucf.Create(_gs, _window, item, this, this, this);
        }

        public int GetSelectedRow(CommunityToolkit.WinUI.UI.Controls.DataGrid dgv)
        {
            if (dgv == null)
            {
                return -1;
        ...
        public void SetSelectedRow(CommunityToolkit.WinUI.UI.Controls.DataGrid dgv, int rid)
        {
            if (dgv == null)
            {
                return;
        ...
        public void SaveChanges()
        {
            ...
            foreach (MemberInfo mem in members)
            {
                ...
                object control = _singleGrid.GetChildNamed(mem.Name + "Edit");

                CheckBoxBase cbox = control as CheckBoxBase;
                if (cbox != null)
                {
                ...
                ComboBoxBase cb = control as ComboBoxBase;
        ...
        private async Task WaitForGridColumns(Type underlyingType, List<IIdName> iidlist)
        {
            ...
            DispatcherQueue.TryEnqueue(async () =>
            {
                ...
                {
                    UIHelper.RemoveDataColumnNamed(_multiGrid, ignored);
                }
                ...
                if (nameMember != null)
                {
                    ...
                    for (int i = 0; i < members.Count; i++)
                    {
                        ...
                        object col = _multiGrid.Columns.FirstOrDefault(x => x.Header != null &&
