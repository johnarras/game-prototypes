...
namespace OxDb.RequestServer.Spawns.Services
{

    public interface IWebSpawnService : IInitializable
    {
        IWebRollHelper GetRollHelper(long entityTypeid);
        Task<List<RewardList>> Roll(WebContext context, long spawnTableId, long rewardSourceId, RollLootArgs rollLootArgs);
        Task<List<RewardList>> Roll(WebContext context, SpawnTable st, long rewardSourceId, RollLootArgs rollLootArgs);
        Task<List<RewardList>> Roll<SI>(WebContext context, List<SI> items, long rewardSourceId, RollLootArgs rollLootArgs) where SI : ISpawnItem;
    }
    ...
    public class WebSpawnService : IWebSpawnService
    {
        ...
        public async Task<List<RewardList>> Roll(WebContext context, long spawnTableId, long rewardSourceId, RollLootArgs rollLootArgs)
        {
            return await Roll(context, _gameData.Get<SpawnSettings>(null).Get(spawnTableId), rewardSourceId, rollLootArgs);
        }

        // Different public roll methods.

        public async Task<List<RewardList>> Roll(WebContext context, SpawnTable st, long rewardSourceId, RollLootArgs rollLootArgs)
        {
            if (st == null)
            {
            ...
            return await Roll(context, st.Items, rewardSourceId, rollLootArgs);
        }

        public async Task<List<RewardList>> Roll<SI>(WebContext context, List<SI> items, long rewardSourceId, RollLootArgs rollLootArgs) where SI : ISpawnItem
        {
            return await InnerRoll(context, items, rewardSourceId, rollLootArgs);
        }

        private async Task<List<RewardList>> InnerRoll<SI>(WebContext context, List<SI> items, long rewardSourceId, RollLootArgs rollLootArgs) where SI : ISpawnItem
        {
            List<RewardList> list = new List<RewardList>();

            list.Add(_rewardService.CreateRewardList(rewardSourceId, new List<Reward>(), rollLootArgs.EntityId));
