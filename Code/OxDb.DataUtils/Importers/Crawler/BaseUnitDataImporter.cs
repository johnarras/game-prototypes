    ...
    public abstract class BaseUnitDataImporter<TParent, TChild> : BaseCrawlerDataImporter<TParent> where TParent : ParentSettings<TChild>, new() where TChild : ChildSettings, IUnitRole, new()
    {
        ...
        protected override async Task<bool> ParseInputFromLines(EditorGameState gs, List<string[]> lines)
        {
            ...
            for (int l = 1; l < lines.Count; l++)
            {
                ...
                UnitImportRow importRow = _importService.ImportLine<UnitImportRow>(gs, l, headers, rowWords, null, true);
        ...
        private void ImportSpawns(EditorGameState gs, long entityTypeId, long entityId, float spawnWeight, string spawnZoneList)
        {
        ...
        private void AddSpawnWeight(long entityTypeId, long entityId, float weight, ZoneType zoneType)
        {
