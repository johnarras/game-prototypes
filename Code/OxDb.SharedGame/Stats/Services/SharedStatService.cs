...
using OxDb.SharedGame.Inventory.Settings.ItemSets;
...
using OxDb.SharedGame.Spells.PlayerData;
using OxDb.SharedGame.Stats.Constants;
using OxDb.SharedGame.Stats.Settings.DerivedStats;
...
using System.Linq;
namespace OxDb.SharedGame.Stats.Services
{
    ...
    public class SharedStatService : IStatService
    {
        ...
        public virtual void CalcStats(Unit unit, bool resetMutableStats)
        {
            ...
            UnitType utype = _gameData.Get<UnitTypeSettings>(unit).Get(unit.EntityId);
            ...
            {
                Set(unit, stat.IdKey, UnitStatValOffsets.Base, coreStatAmount);

                if (monsterScalePercent != 0)
                {
                    Add(unit, stat.IdKey, UnitStatValOffsets.Pct, monsterScalePercent);
                }
            }
            ...
            foreach (StatType ms in mutableStats)
            {
                if (ms.MaxPool > 0)
                {
                    Set(unit, ms.IdKey, UnitStatValOffsets.Base, ms.MaxPool);
                }
            }
            ...
            {
                foreach (Effect eff in utype.Effects)
                {
                ...
                foreach (long setId in setQuantities.Keys)
                {
                    ...
                    if (setType.Stats != null)
                    {
                        foreach (SetStat sb in setType.Stats)
                        {
                            ...
                            {
                                Add(unit, sb.StatTypeId, UnitStatValOffsets.Base, levelData.StatAmount * sb.Percent / 100);
                            }
                        }
                    }
            ...
            if (charmStatus != null)
            {
                ...
                if (bonusList != null)
                {
                    foreach (PlayerCharmBonus charmBonus in bonusList.Bonuses)
                    {
                        ...
                        {
                            Add(unit, charmBonus.EntityId, UnitStatValOffsets.Pct, charmBonus.Quantity);
                        }
                    }
                }
            }
            ...
            if (unit.StatPct != 0 && unit.StatPct != 100)
            {
                ...
                {
                    Set(unit, statTypeId, UnitStatValOffsets.Base, unit.Stats.Pct(statTypeId) * unit.StatPct / 100);
                }
            }
            ...
            foreach (StatType stat in mutableStats)
            {
                ...
                Set(unit, stat.IdKey, UnitStatValOffsets.Curr, newValue);
        ...
        protected virtual void AddDerivedStats(Unit unit, RpgLevel levData)
        {
            ...
            for (int d = 0; d < list.Count; d++)
            {
                ...
                Add(unit, ds.ToStatTypeId, UnitStatValOffsets.Base, addval);
            }
        }
        ...
        protected void AddEffectStat(Unit unit, IEffect e, RpgLevel levData, int multiplier)
        {
            ...
            {
                Add(unit, e.EntityId, UnitStatValOffsets.Base, e.Quantity * multiplier);
            }
            else if (e.EntityTypeId == EntityTypes.StatPct)
            {
                Add(unit, e.EntityId, UnitStatValOffsets.Pct, e.Quantity * multiplier);
            }
        }
