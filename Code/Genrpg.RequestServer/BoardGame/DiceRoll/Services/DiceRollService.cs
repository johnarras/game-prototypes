...
using Genrpg.Shared.BoardGame.Activations.Settings;
...
namespace Genrpg.RequestServer.BoardGame.DiceRoll.Services
{
    ...
    public class DiceRollService : IDiceRollService
    {
        ...
        private IRepositoryService _repoSevice = null;
        ...
        public async Task RollDice(WebContext context, DiceRollParams rollData)
        {
            try
            {
                ...
                boardData.PVPData = await _repoSevice.Load<PvpSharedData>(boardData.OwnerId);
                ...
                {
                    if (userData.Coins.Get(UserCoinTypes.Energy) < userData.Vars.Get(UserVars.PlayMult))
                    {
                        args.Response.DiceRollResult = DiceRollResults.OutOfDice;
                        context.Responses.AddResponse(args.Response);
                        return;
                    }
                    else
                    {
                        userData.Coins.Add(UserCoinTypes.Energy, -userData.Vars.Get(UserVars.PlayMult));
                        args.Response.DiceCost = userData.Vars.Get(UserVars.PlayMult);
                    }
                }
                ...
                await _rewardService.GiveRewardsAsync(context, rewardData.Rewards, null);
                    ...
                    userData.Coins.Get(UserCoinTypes.Energy) < energyStorage)
                {
                    ...
                    context.Responses.AddResponse(new UpdateUserEnergyResponse() { EnergyAdded = 0, LastHourlyReset = userData.LastHourlyReset });
                }

                await _boardGenService.SetBoardTiles(context, boardData, args.Response.InitialIndex, args.Response.TilesIndexesReached.Count, true);
        ...
        protected async Task GetRewards(WebContext context, RollDiceArgs args)
        {
            ...
            for (int step = 0; step < args.Response.TilesIndexesReached.Count; step++)
            {
                ...
                if (args.Mode.GiveDefaultTileRewards)
                {
                    ...
                    List<RewardList> rewards = await _spawnService.Roll(context, tileType.Rewards, rollData);

                    rollStep.Rewards.AddRange(rewards);
                }

                await AddExtraPrizes(context, args, rollStep, tileIndex, args.Board.PassPrizes, RewardSources.PassTile);

                if (landing)
                {
                    await AddExtraPrizes(context, args, rollStep, tileIndex, args.Board.LandPrizes, RewardSources.LandTile);
                }
            }
        }

        private async Task AddExtraPrizes(WebContext context, RollDiceArgs args, RollStep rollStep, int tileIndex, 
            SmallIdShortCollection boardPrizes, long rewardSourceId)
        {

            BoardPrize extraPrize = _gameData.Get<BoardPrizeSettings>(context.user).Get(boardPrizes.Get(tileIndex));
            ...
            if (extraPrize != null)
            {
                ...
                List<RewardList> rewardLists = await _spawnService.Roll(context, extraPrize.LootTable, rollData);
                if (rewardLists.Count > 0)
                {
                    rollStep.Rewards.AddRange(rewardLists);
                }
                boardPrizes.Set(tileIndex, 0);
            }
        }
        ...
        private async Task UpdateAfterRoll(WebContext context, RollDiceArgs args)
        {
            ...
            if (nextData != null && nextData.NextBoard != null)
            {
                ...
                context.Set(nextData.NextBoard);
