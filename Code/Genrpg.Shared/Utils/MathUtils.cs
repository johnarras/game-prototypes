...
namespace Genrpg.Shared.Utils
{
    ...
    public class MathUtils
    {
        ...
        public static double LPNorm(double power, params double[] vals)
        {
            if (vals == null || vals.Length < 1 || power < 0.5f)
            {
                return 0.0f;
            }

            // Bad limit tending to infinity, call it the max. 
            if (power > 10)
            {
                return vals.Max(x => Math.Abs(x));
            }

            return Math.Pow(vals.Sum(x => Math.Pow(Math.Abs(x), power)), 1 / power);
        }

        public static double LPNorm(double power, params int[] vals)
        {
            return LPNorm(power, vals.Select(x => (double)x).ToArray());
        }

        public static double PythagoreanDistance(params double[] vals)
        {
            return LPNorm(2, vals);
        }

        public static double HammingDistance(params double[] vals)
        {
