...
namespace Genrpg.Shared.Inventory.Services
{
    ...
    public class ItemGenService : IItemGenService
    {
        ...
        private IBinarySerializer _serializer = null;

        // ACtually generate an item from either an old item or an itemTypeId.
        public virtual Item Generate(IRandom rand, ItemGenArgs genData)
        {
            Item item = null;
        ...
        public Item CreateSimpleItem(IRandom rand, ItemGenArgs gd)
        {
            ...
            if (itype.Effects != null)
            {
                ...
                foreach (ItemEffect eff in itype.Effects)
                {
                    ...
                    item.Effects.Add(_serializer.MakeCopy(eff));
                }
            }
            return item;
        }
        ...
        public Item GenerateLevelRangeItem(IRandom rand, ItemGenArgs gd)
        {
            ItemType itype = _gameData.Get<ItemTypeSettings>(null).Get(gd.ItemTypeId);
            if (itype == null)
            {
        ...
        public ItemNameResult GenerateItemName(IRandom rand, long itemTypeId, int level, long qualityTypeId, List<FullReagent> reagents)
        {
            ...
            {
                result.SingularName = badPrefix + " " + singularItemName;
                result.PluralName = badPrefix + " " + pluralItemName;
                return result;
            }
            ...
            if (qualityTypeId <= QualityTypes.Uncommon || rand.Next() % 10 == 4)
            {
                if (rand.Next() % 2 == 0)
                {
                    result.SingularName = adj1 + " " + singularItemName;
                    result.PluralName = adj1 + " " + pluralItemName;
                    return result;
                }
                else
                {
            ...
            if (rand.Next() % 3 != 2)
            {
                result.SingularName = adj1 + " " + singularItemName + " of " + suffixName;
                result.PluralName = adj1 + " " + pluralItemName + " of " + suffixName;
                return result;
            }
            // Name of Adj Noun
            else if (rand.Next() % 2 == 1)
            {
                ...
                result.SingularName = singularItemName + " of " + theSuffix + adj2 + " " + suffixName;
                result.PluralName = pluralItemName + " of " + theSuffix + adj2 + " " + suffixName;
                return result;
            }
            // Adj name of Adj Noun
            else
            {
                ...
                result.SingularName = adj1 + " " + singularItemName + " of " + theSuffix + adj2 + " " + suffixName;
                result.PluralName = adj1 + " " + pluralItemName + " of " + theSuffix + adj2 + " " + suffixName;
                return result;
            }

        }

        public Item GenerateEquipment(IRandom rand, ItemGenArgs genData)
        {
            ItemType itype = _gameData.Get<ItemTypeSettings>(null).Get(genData.ItemTypeId);
            if (itype == null)
            {
        ...
        public long ChooseItemQuality(IRandom rand, ItemGenArgs genData)
        {
        ...
        protected void AddItemReagentFromReagent(IRandom rand, Reagent reagent, List<FullReagent> reagents, ItemGenArgs gd)
        {
