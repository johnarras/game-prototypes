    ...
    where TInitData : DragItemInitData<TData, TDragItem, TScreen, TInitData>
{
    ...
    public virtual void OnUpdateChild(object childObject)
    {
    ...
    public virtual void OnPointerUp()
    {
        ...
        foreach (GraphicRaycaster gr in raycasters)
        {
            ...
            OtherIconTarget otherTarget = null;
            ...
            if (results.Count > 0)
            {
                foreach (RaycastResult res in results)
                {
                    ...
                    if (otherTarget == null)
                    {
                        otherTarget = _clientEntityService.FindInParents<OtherIconTarget>(res.gameObject);
                    }
                }
                if (results.Count > 0)
                {
                ...
                if (otherTarget != null)
                {
                    HandleOtherTarget(this as TScreen, _dragItem, otherTarget, lastTargetHit);
                }
                didHitItem = true;
            }
    ...
    protected virtual void HandleOtherTarget(TScreen startSc, TDragItem dragItem, OtherIconTarget otherTarget, GameObject finalObjectHit)
    {
