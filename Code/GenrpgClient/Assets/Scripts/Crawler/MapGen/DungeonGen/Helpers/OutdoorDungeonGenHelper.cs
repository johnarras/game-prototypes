...
{
    public class OutdoorDungeonGenHelper : BaseDungeonGenHelper
    {
        public override long HelperKey => DungeonTypes.Outdoors;
        ...
        public override async ValueTask<bool> GenerateLevel(CrawlerMapGenData genData, DungeonLevelGenArgs levelArgs)
        {
            ...
            foreach (ConnectedPairData pairData in roadsToMake)
            {
                ...
                long zoneTypeId = levelArgs.Rand.NextDouble() < 0.5f ? ZoneTypes.Road : levelArgs.Map.ZoneTypeId;

                _mapGenService.ConnectPairOfPoints(levelArgs.Map, pairData, 2, new List<long>(), zoneTypeId, levelArgs.Rand);
            }

            _mapGenService.AddSmallRoomsAndBlankSpaces(levelArgs);

            _mapGenService.RemoveEdgePoints(levelArgs.Map, 1);
