...
public interface IRealtimeNetworkService : IInitializable, IMapTokenService
{
    ...
    void HandleOneMapApiMessage(IMapApiMessage message, CancellationToken token);
...
public class RealtimeNetworkService : IRealtimeNetworkService
{
    ...
    private EMapApiSerializers _serializerType = EMapApiSerializers.Json;

    public void SetRealtimeEndpoint(string host, long port, EMapApiSerializers serializer)
    {
        _host = host;
        _port = port;
        _serializerType = serializer;
    }
    ...
    public void HandleOneMapApiMessage(IMapApiMessage message, CancellationToken token)
    {
        try
        {
            if (message == null)
            ...
            if (message is InfrequentMessageEnvelope envelope &&
                envelope.InfrequentApiMessage != null)
            {
                message = envelope.InfrequentApiMessage;
            }
