...
namespace Assets.Scripts.BoardGame.Controllers
{
    ...
    public interface IBoardGameController : IInitializable
    {
        ...
        void RollDice();
        void SetTiles(List<ClientTile> tiles);
        ...
        IReadOnlyList<ClientTile> GetTiles();
        ClientTile GetTile(long indeX);
    ...
    public class BoardGameController : IBoardGameController
    {
        ...
        private List<ClientTile> _tiles = new List<ClientTile>();
        ...
        public void SetTiles(List<ClientTile> tiles)
        {
            _tiles = tiles.OrderBy(x=>x.GeTTileIndex()).ToList();  
        }

        public IReadOnlyList<ClientTile> GetTiles()
        {
            return _tiles;
        }

        public ClientTile GetTile(long index)
        {
        ...
        {
            await Task.CompletedTask;
            _assetService.LoadAssetInto(GetBoardAnchor(), AssetCategoryNames.Tiles, tti.TileType.Art, OnLoadTile, tti, token);
        }

        private void OnLoadTile(object obj, object data, CancellationToken token)
        {
            ...
            ClientTile newTile = go.GetComponent<ClientTile>();
            ...
            TileTypeWithIndex tti = data as TileTypeWithIndex;
            ...
            newTile.InitData(tti);

            ClientTile currTile = GetTile(tti.Index);

            if (currTile != null)
            {
                _clientEntityService.Destroy(currTile.gameObject);
                _tiles.Remove(currTile);
            }

            newTile.name = newTile.name + tti.Index;

            float height = _terrainManager.GetInterpolatedHeight(tti.XPos, tti.ZPos);
            newTile.transform.position = new Vector3(tti.XPos, height + 0.3f, tti.ZPos);
        ...
        {
            ClientTile controller = GetTile(index);
            ...
            controller.GetTTI().TileType = _gameData.Get<TileTypeSettings>(_gs.user).Get(tileTypeId);
            SetTile(controller.GetTTI(), token);
        }
