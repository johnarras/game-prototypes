...
namespace Assets.Scripts.BoardGame.Loading.Steps
{
    public class AddBoardTrees : BaseLoadBoardStep
    {
        ...
        public override ELoadBoardSteps Key => ELoadBoardSteps.AddTrees;
        public override async Awaitable Execute(BoardData boardData, CancellationToken token)
        {
            ...
            IReadOnlyList<ClientTile> tiles = _boardController.GetTiles();
            ...
            currentTypes = currentTypes.OrderBy(x=>HashUtils.NewUUId()).ToList();       
            ...
            {
                Vector3 pos = (Vector3)tile.transform.position;
                int cx = (int)pos.x;
                int cz = (int)pos.z;
