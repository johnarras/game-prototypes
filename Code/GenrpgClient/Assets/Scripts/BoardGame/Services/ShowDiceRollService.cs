...
namespace Assets.Scripts.BoardGame.Services
{
    ...
    public class ShowDiceRollService : IShowDiceRollService
    {
        ...
        private ILockoutManager _lockoutManager = null;
        ...
        public async Awaitable ShowDiceRoll(RollDiceResponse result, CancellationToken token)
        {
            ...
            RewardParams rp = new RewardParams() { SkipVisualUpdate = true };
            ...
            _dispatcher.Dispatch(new ShowFloatingText(_gameData.Get<BoardGameSettings>(_gs.ch).FavoriteSuperhero, EFloatingTextArt.Error));
            for (int step = 0; step < result.TilesIndexesReached.Count; step++)
            {
                int tileIndex = result.TilesIndexesReached[(int)step];

                Vector3 startPos = player.transform.position;
                ClientTile endTile = _boardGameController.GetTile(tileIndex);
                _clientEntityService.AddToParent(player, endTile);
                ...
                {
                    _rewardService.GiveRewards(_rand, _gs.ch, currStep.Rewards, rp);
                    foreach (RewardList rlist in currStep.Rewards)
                    {
                        foreach (IReward reward in rlist.Rewards)
                        {
                            endTile.ShowReward(reward);
                        }
                    }
                }
            }
