...
public class FileDownloadService : IFileDownloadService
{
    ...
    private async Awaitable DownloadFileInternal(InternalFileDownload fileDownload, CancellationToken token)
    {
        ...
        if (_downloading.ContainsKey(fileDownload.FilePath))
        {
            ...
            {
                if (fd.DownloadData == null || fd.DownloadData.Handler == null)
                {
                    continue;
                }
    ...
    public void DownloadTypedFile<T>(string url, Action<T> handler, EDataCategories category, CancellationToken token) where T : class
    {
        ...
        DownloadFileData fileData = new DownloadFileData()
        {
            ...
            Handler =
              (obj, data, token) =>
              {
                  ...
                  T t = _serializer.Deserialize<T>(obj.ToString());
                  handler(t);
              },
        };
